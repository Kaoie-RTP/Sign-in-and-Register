...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const setupPasswordToggle = (inputId: string, btnId: string) => {
  ...
  const translatePage = (lang: string) => {
    ...
    if (h1) {
      if (h1.textContent?.toUpperCase().includes('SIGN')) h1.textContent = translations[lang].signin;
      if (h1.textContent?.toUpperCase().includes('REGISTER')) h1.textContent = translations[lang].register;
    }
    ...
    if (confirmPasswordLabel) confirmPasswordLabel.textContent = translations[lang].confirm_password;
    const emailLabel = document.querySelector('label[for="email"]');
    if (emailLabel) emailLabel.textContent = translations[lang].email;
    
    const usernameInput = document.getElementById('username') as HTMLInputElement | null;
    if (usernameInput) usernameInput.placeholder = translations[lang].usernamePh;
    const passwordInput = document.getElementById('password') as HTMLInputElement | null;
    if (passwordInput) passwordInput.placeholder = translations[lang].passwordPh;
    const confirmPasswordInput = document.getElementById('confirm_password') as HTMLInputElement | null;
    if (confirmPasswordInput) confirmPasswordInput.placeholder = translations[lang].confirmPasswordPh;
    const emailInput = document.getElementById('email') as HTMLInputElement | null;
    if (emailInput) emailInput.placeholder = translations[lang].emailPh;
    
    const signInBtn = document.querySelector('button[type="submit"]');
    if (signInBtn && signInBtn.textContent?.toLowerCase().includes('sign')) signInBtn.textContent = translations[lang].signInBtn;
    if (signInBtn && signInBtn.textContent?.toLowerCase().includes('register')) signInBtn.textContent = translations[lang].registerBtn;
    const togglePasswordBtn = document.getElementById('toggle-password');

    if (togglePasswordBtn) togglePasswordBtn.textContent = translations[lang].show;
    const toggleConfirmPasswordBtn = document.getElementById('toggle-confirm-password');
    if (toggleConfirmPasswordBtn) toggleConfirmPasswordBtn.textContent = translations[lang].show;
    
    const toRegister = document.querySelector('a[href="register.html"]');
    if (toRegister) toRegister.textContent = translations[lang].toRegister;
    const toSignIn = document.querySelector('a[href="signin.html"]');
    if (toSignIn) toSignIn.textContent = translations[lang].toSignIn;
  }
